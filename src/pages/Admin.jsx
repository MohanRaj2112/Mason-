        ...
        if (projRes.status === 'fulfilled' && projRes.value.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            setProjects(data.map(item => ({
              ...item,
              id: item.projectId || item._id || item.id
            })));
          }
        }
        if (srvRes.status === 'fulfilled' && srvRes.value.ok) {
          const data = await srvRes.value.json();
          if (Array.isArray(data) && data.length > 0) {
            setServices(data.map(item => ({
              ...item,
              id: item.serviceId || item._id || item.id
            })));
          }
        }
        if (cRes.status === 'fulfilled' && cRes.value.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            setMessages(data.map(item => ({
              ...item,
              id: item.contactId || item._id || item.id
            })));
          }
        }
        if (uRes.status === 'fulfilled' && uRes.value.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            setCustomers(data.map(item => ({
              ...item,
              id: item.userId || item._id || item.id,
              location: item.location || 'Salem / Coimbatore'
            })));
          }
        }
=======
...
<<<<<<< HEAD
  const handleEditToolSubmit = async (e) => {
    ...
    const targetId = editingTool._id || editingTool.toolId || editingTool.id;
    const updated = tools.map(t => ((t._id === targetId || t.toolId === targetId || t.id === targetId) ? {
      ...editingTool,
...
<<<<<<< HEAD
    ...
    try {
      ...
      console.warn('Tool edit sync notice:', err);
    }
  };
...
<<<<<<< HEAD
    ...
    try {
      ...
      console.warn('Project create sync notice:', err);
    }
  };
...
<<<<<<< HEAD
    ...
    try {
      ...
      console.warn('Project edit sync notice:', err);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    if (deletingItem.type === 'tool') {
      const updated = tools.filter(t => t._id !== deletingItem.id && t.toolId !== deletingItem.id && t.id !== deletingItem.id);
      setTools(updated);
      showToast(`Removed "${deletingItem.name}" from inventory.`, 'success');
      try {
        await fetch(`/api/tools/${deletingItem.id}`, { method: 'DELETE' });
      } catch {}
    } else if (deletingItem.type === 'project') {
      const updated = projects.filter(p => p.id !== deletingItem.id && p.projectId !== deletingItem.id && p._id !== deletingItem.id);
      setProjects(updated);
      showToast(`Removed project "${deletingItem.name}".`, 'success');
      try {
        await fetch(`/api/projects/${deletingItem.id}`, { method: 'DELETE' });
      } catch {}
    } else if (deletingItem.type === 'booking') {
      const updated = bookings.filter(b => b.bookingId !== deletingItem.id && b._id !== deletingItem.id);
      setBookings(updated);
      showToast(`Deleted booking record ${deletingItem.id}.`, 'success');
      try {
        await fetch(`/api/bookings/${deletingItem.id}`, { method: 'DELETE' });
      } catch {}
    } else if (deletingItem.type === 'message') {
      const updated = messages.filter(m => m.id !== deletingItem.id && m.contactId !== deletingItem.id && m._id !== deletingItem.id);
      setMessages(updated);
